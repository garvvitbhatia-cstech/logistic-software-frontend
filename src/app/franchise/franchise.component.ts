...
export class FranchiseComponent implements OnInit,OnDestroy {
  ...
  Create(){
    ...
    if(this.action == 'Edit'){
      this.appService.postData('franchise/update',data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
        var r:any=res;
        $('#addUserBtn').html('Save');
        if(r.success){
          this.getList();
  ...
  setHeading(){
    this.action = 'Add';
    this.heading = 'Add New Franchise';
    this.cleanForm();
  }
  getUser(id:number){
    this.heading = 'Edit Franchise';
    this.action = 'Edit';
    const data = {
      token: localStorage.getItem('token'),
      id: id
    };
    this.appService.postData('franchise/get',data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      if(r.success){
        $('#r_id').val(r.user.id);
  ...
  updateStatus(userID:string,status:string){
    const data = {};
    this.appService.putData('franchise/status/update/'+userID+'/'+status,data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      this.getList();
    },error=>{
      this.toastr.error("Server Error","Error");
